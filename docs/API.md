...
- `quarantine`：隔离停用；ACTIVE → QUARANTINED；最低角色 OPERATOR。业务输入：无，但必须填写 remark。
- `retire`：报废；ACTIVE / QUARANTINED → RETIRED；最低角色 REVIEWER。业务输入：无，但必须填写 remark。

### 校准任务 `jobs`

- `start`：开始校准；DRAFT → RUNNING；最低角色 OPERATOR。业务输入：无，但必须填写 remark。
- `evaluate`：计算判定；RUNNING → REVIEW；最低角色 OPERATOR。业务输入：无，但必须填写 remark。
- `approve`：复核签发；REVIEW → APPROVED；最低角色 REVIEWER；禁止创建者自审。业务输入：无，但必须填写 remark。
- `reject`：退回重测；REVIEW → RUNNING；最低角色 REVIEWER；禁止创建者自审。业务输入：无，但必须填写 remark。

资金类动作仅保存企业确认的结算凭证，并不连接支付机构。系统不会自动读取或上传外部账户数据。

- `jobs/cancel`：撤销校准任务，DRAFT/RUNNING/REVIEW → CANCELLED；角色 REVIEWER，禁止创建者自审；data 字段：reason（撤销原因）。

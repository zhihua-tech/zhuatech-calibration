...
const title='知华计量校准管理', subtitle='器具台账、量值溯源、测量判定与证书';
const token=ref(sessionStorage.getItem('calibration-session')||''),user=ref(null),catalog=ref(null),dashboard=ref(null);
...
async function request(path,{method='GET',body,key,raw=false}={}){
 ...
 if(!r.ok){let m='请求失败';try{m=(await r.json()).message||m;}catch{}if(r.status===401){token.value='';user.value=null;sessionStorage.removeItem('calibration-session');}throw new Error(m);}
...
async function login(){await guarded(async()=>{const r=await request('/auth/login',{method:'POST',body:{username:username.value,password:password.value}});token.value=r.token;user.value=r.user;sessionStorage.setItem('calibration-session',r.token);password.value='';await load();});}
async function logout(){await guarded(async()=>{await request('/auth/logout',{method:'POST',body:{}});token.value='';user.value=null;sessionStorage.removeItem('calibration-session');});}
...
async function exportCsv(){await guarded(async()=>{const blob=await request('/export/'+section.value,{raw:true});download(blob,'calibration-'+section.value+'.csv');});}
...
<template>
 ...
 <div v-else class="app-shell" :style="{'--accent':catalog?.accent}">
  <aside class="sidebar">
   <div class="brand"><span class="brand-mark">知</span><div><strong>{{title.replace('知华','')}}</strong><small>ZHUATECH · CALIBRATION</small></div></div>
